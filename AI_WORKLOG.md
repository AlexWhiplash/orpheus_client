...
**Дальше по плану:** релиз 1.1.7+21 (бамп, APK, админка `required=false`, регистрация версии,
проверка на Xiaomi), потом хвосты: lockout-алерт на сервере, rate-limit, HTTP-PoP для `/api/signal`,
session takeover, кто долбит `/api/support/stats` с 401.
