...
## 2026-07-30 — Тест-сборка 1.1.8+51 [wl/dev]

Бамп 50 → 51 после мержа двух правок в `wl/dev`: след пропущенного звонка из duress-сессии (PR #2,
два коммита — закрепление тишины тестами + сам след) и нумерация шагов мастера настройки (PR #1).
Собрано с обязательным `--target-platform android-arm,android-arm64` (см. релизный чеклист).
`debugFileLogging` осознанно остаётся `true`: парк тестовый.

**Что просить у тестировщиков ИМЕННО на этой сборке** (нужен второй телефон — сценарий впервые
проверяем на устройстве): позвонить на телефон, где активна duress-сессия → под принуждением НЕ
должно быть ни звонка, ни отметки → войти настоящим PIN → в переписке непрочитанный «Пропущенный
звонок» со временем ЗВОНКА, а не разблокировки. Рядом тем же заходом закрывается давний долг:
сообщение, пришедшее под duress, доезжает после входа настоящим PIN.

---

## 2026-07-30 — Звонок под duress: разбор по коду, моя прошлая версия ОПРОВЕРГНУТА [wl/dev]

**Снимаю утверждение из отчёта 30.07** («в пути входящего звонка гейта duress нет вообще; телефон
звонит на пустом профиле, ответ соединяет наблюдателя с живым контактом»). Оно было основано на
grep'е по `isDuressMode` — в файлах звонка слова действительно нет, но вывод из этого неверный.
Разбор по коду даёт обратное: **под duress входящий звонок не поднимается вообще.**

**Чем это держится (проверено по коду):**
- `IncomingMessageHandler.handleDecoded` гейтит ВСЁ, кроме teardown-сигналов, строгим mutual-add:
  `!await _db.isContact(senderKey)` (incoming_message_handler.dart:128). А `DatabaseService.isContact`
  под duress возвращает false для любого адреса (database_service.dart:489). Значит `call-offer`
  умирает ДО резолва имени (:161), до буфера сигналинга (:159) и до показа CallKit (:197).
- Второй дороги через push-изолят нет: `_evaluate` держит сервисный сокет ЗАКРЫТЫМ, пока свеж
  heartbeat main-изолята (`kPrefMainAliveTs`, бит каждые 4с, порог 12с —
  push_connection_service.dart:55/384-393). Живая duress-сессия = живой main = сервис молчит. Когда
  main умирает, duress-сессии уже нет (флаг только в RAM), и звонок из изолята — штатное поведение
  настоящего аккаунта, а не утечка.
- Ответ на звонок внутри duress закрыт ещё 26.07: resumed-ветки гейтятся на `isDuressMode`
  (main.dart:1171/1178), pending-звонок в duress не обрабатывается, а к возврату настоящим PIN он
  протухает (TTL 30с).
- Единственный вход в обработчик — `handleRawMessage`/`handleDecoded`; второй вызов
  `handleDecoded` (слив очереди push-изолята, main.dart:656) сам гейтится по duress. HTTP-fallback
  сигналинга — только на ОТПРАВКУ, приёмного пути в обход нет.

**Что сделано (это и есть работа):** свойство ПОБОЧНОЕ — гейт писался под mutual-add, про duress он
не знает, и ни один тест его не проверял. Любая правка mutual-add (например «принимать звонки от
не-контактов») молча поднимет звонок от реального контакта на пустом профиле наблюдателя. Закрепил
на двух уровнях:
- `database_service_test`: `isContact` под duress → false для СУЩЕСТВУЮЩЕГО контакта, вне duress →
  true (контроль), и контакт не удалён.
- `incoming_message_handler_test`: `call-offer` от реального контакта при `isContact == false` не
  открывает экран, не показывает уведомление, НЕ резолвит имя (новый спай `nameLookups` в фейке БД),
  не создаёт буфер сигналинга — проверено в обеих ветках показа (foreground и background; фоновая
  сама себе детектор: прорыв гейта дал бы MissingPluginException на плагине CallKit). Контроль: вне
  duress тот же кадр звонок поднимает.
- **Проверено мутацией:** снял `_isDuressMode ||` в `isContact` и обнулил гейт в хендлере — падают
  оба новых теста (и попутно старый mutual-add). Без этой проверки тесты были бы вакуумны.
- Инвариант с ценой записан в `CLAUDE.md` (раздел «Безопасность»).

**СЛЕД ПРОПУЩЕННОГО — СДЕЛАН (решение владельца 30.07: «след после настоящего PIN»).** До этого
звонок под duress исчезал совсем: ни отметки в переписке, ни уведомления после входа настоящим PIN,
а звонивший видел обычное «не ответили». Тот же класс потери, что для сообщений закрыли парковкой.
- Парковать сам `call-offer` НЕЛЬЗЯ (SDP живёт меньше минуты, после разблокировки бесполезен, а
  handler на нём поднял бы экран звонка). Поэтому `parkIncomingUntilRealSession` (переименован из
  `parkPersonalChatUntilRealSession` — теперь паркует не только chat) кладёт в `PendingInboxStorage`
  МАРКЕР: `{type:'missed-call', sender_pubkey, message_id:'missed-<call_id>', server_ts_ms}`.
  SDP/ICE на диск не попадают — только адрес и время, то есть ровно то, что окажется в переписке.
- `message_id` даёт дедуп на ДВУХ уровнях бесплатно: `append` гасит повтор в очереди, а
  `messageExistsByMessageId` — повторный разбор очереди.
- Разбор — в `handleDecoded` за общим гейтом mutual-add (маркер от не-контакта не дойдёт), пишет
  `ChatMessage(text:'Missed call', isSentByMe:false, isRead:false, timestamp: время ЗВОНКА)` и зовёт
  `emitChatUpdate`. Уведомление не показываем: очередь разбирается сразу после входа настоящим PIN,
  человек уже в приложении. Текст-маркер уже входит в `callEventTexts`, поэтому статистика профиля
  не посчитает его перепиской, а `chat_screen` рисует иконкой пропущенного.
- **Новый параметр `handleDecoded(..., fromPendingQueue: false)`:** маркер принимается ТОЛЬКО из
  очереди. Иначе кадр `type:'missed-call'` из сети позволял бы дописывать записи в чужую историю
  звонков — свой синтетический тип нельзя молча пускать по общему сетевому пути.
- Тесты (+6, всего 444): маркер пишет непрочитанную запись с временем звонка; повторный разбор не
  двоит; из сети игнорируется; от не-контакта отсекается mutual-add'ом; парковка кладёт маркер БЕЗ
  SDP (проверен точный набор ключей) и не копит дубли. **Мутацией проверены все три гарда** (гард
  `fromPendingQueue`, дедуп, сама ветка парковки) — падают 4 теста из 4 соответствующих.
- Обновлён тест, который закреплял ПРЕЖНЕЕ поведение («call-offer не откладывается») — теперь он про
  остальные не-chat кадры (`hang-up`, `call-rejected`, `room-message`, `pong`).

**Статус:** analyze — 0 ошибок; test — 444 passed (было 436).

---

## 2026-07-30 — Хвосты 1.1.8: PR в upstream, нумерация шагов мастера, ревизия чеклиста [wl/dev]

**Задача:** свести релизный чеклист 1.1.8 к нулю после b50 и передать работу в upstream.

**Сделано:**
- PR в upstream создан: [vadimkolobanov/orpheus_client#30](https://github.com/vadimkolobanov/orpheus_client/pull/30),
  +48→+50 (четыре коммита: фиксы звонка, ARM-only сборка, бамп, публичный changelog). Предыдущий
  PR #29 закрывал +43→+48, то есть фиксы звонка Вадиму переданы не были.
- **Найден и исправлен баг в диффе +49:** шаг «Разрешите полноэкранные оповещения» в
  `showSetupDialog` получил жёстко зашитый `number: 2`, и шаг автозапуска — тоже `2`, а шаг
  «Проверьте настройки уведомлений» считал только китайский OEM (`? 3 : 2`). На Android 14+ без
  разрешения список нумеровался **1, 2, 2** (и 1, 2, 2, 3 на Xiaomi) — то есть ровно в том случае,
  для которого шаг и добавлен. Номера теперь выводятся один раз из двух условий.

**Ревизия чеклиста — что осталось незакрытым (не мои руки):**
- **Фаза 5 не пройдена:** у записи b50 в админке `public_changelog` = заглушка «Повышена
  стабильность работы приложения», а полный текст на 8 пунктов лежит в `PUBLIC_CHANGELOG_RU.md`.
  Проверено по проду: `GET /api/public/releases?limit=3`. Правится в админке
  (`PUT /admin/versions/{id}`, JWT-логин).
- **Серверная мина в том же месте:** `POST /admin/set-version` (путь по `x-admin-secret`) принимает
  в модели `public_changelog` и `internal_notes`, но в `insert().values(...)` их не кладёт —
  оба поля молча теряются (`main.py:2047-2058`). Кто заливает версию этим эндпоинтом, changelog
  не зальёт никогда и об этом не узнает.
- **Этап C инертен:** `CHAT_FENCE_ENABLED` на сервере дефолт `0`, флаг не включён. `OFFLINE_CHAT_TTL_DAYS`
  в коде с дефолтом 21 — как договаривались.
- `debugFileLogging = true` — осознанно (парк тестовый), для публичного релиза обязан стать `false`.

**ОПРОВЕРГНУТО В ЭТОТ ЖЕ ДЕНЬ — см. запись «Звонок под duress» выше. Абзац оставлен как история
ошибки: вывод сделан из grep'а, гейт лежит уровнем ниже (mutual-add), звонок НЕ поднимается.**

**Находка вне диффа (в отчёт владельцу, не правил):** в пути входящего ЗВОНКА гейта duress нет
вообще — ни в `incoming_message_handler`, ни в `notification_service`, ни в `call_state_service`
(grep по `isDuressMode` даёт только БД, `rooms_service`, `support_chat_service`, `main.dart`,
экраны настроек). Личный `chat` под duress откладывается, уведомления комнат гасятся, а `call-offer`
проходит штатно: телефон звонит на пустом профиле, и ответ соединяет наблюдателя с живым контактом.
Решение неочевидно (тихий дроп = потеря настоящего звонка), поэтому вынесено владельцу как выбор.

**Статус:** analyze — 0 ошибок (567 info, включая три `avoid_print` в `tools/`); test — 436 passed.

---

## 2026-07-27 — Тест-сборка 1.1.8+50 (замена b49) [wl/dev]

Бамп 49 → 50 и пересборка: b49 собиралась ДО фиксов звонка, а владелец выкатывает обновление
всем. b49 никуда не деплоилась, поэтому замена без последствий. Собрано с обязательным
`--target-platform android-arm,android-arm64` (см. релизный чеклист) — 101.4 МБ; подпись прежняя
(3734..2cca), ставится обновлением поверх b44.

`AppConfig.debugFileLogging` осознанно оставлен `true`: весь парк — тестовый, а файловый лог это
единственный способ разобрать инцидент вроде сегодняшнего (у получательницы лога звонка не
оказалось именно потому, что прислала выгрузку из памяти). Для ПУБЛИЧНОГО релиза флаг обязан быть
`false` — пункт есть в чеклисте.

---

## 2026-07-27 — Инцидент звонка: чёрный экран у получателя + задвоение уведомлений [wl/dev]

Живой тест: владелец (b49) звонит второму телефону (b44). Четыре жалобы. Разбор 8 агентами по
коду и логу, но **решающие данные дал сам получатель словами** — не лог.

**Мои первые версии были неверны, снимаю их.** По «собеседник показан офлайн, звонок не прошёл» я
подозревал презенс и маршрутизацию. Ни то, ни другое: получательница написала «входящий звонок был,
но экран был чёрный; разблокировала, вижу что звонит Орфей, нажимаю на трубку — открылась страница
настроек». Доставка сработала штатно, презенс показывал офлайн ПРАВИЛЬНО (её приложение было убито,
жив только push-сокет), а сломан был ПОКАЗ.

**Корень (подтверждён по коду):** на Android 14+ разрешение `USE_FULL_SCREEN_INTENT` больше не
выдаётся автоматически. Без него входящий не разворачивается поверх локскрина, а
`CallkitIncomingActivity.onCreate` (третьесторонний плагин, наш форк) при отсутствии разрешения САМ
открывает `Settings.ACTION_MANAGE_APP_USE_FULL_SCREEN_INTENT` — это и увидела пользователь вместо
звонка. При этом в плагине есть готовые `canUseFullScreenIntent()` и `requestFullIntentPermission()`,
а мы их **не вызывали ни разу** (grep по lib/ пуст). То есть человек узнавал о недостающем
разрешении ровно в тот момент, когда ему звонили и он не мог ответить.
Фикс: проверка добавлена в `needsManualSetup()` (то есть мастер настройки сам всплывёт) и отдельным
шагом в диалог настройки, открывающим нужный системный экран одним нажатием.

**Задвоение уведомлений — наш же регресс, накопленный двумя правками.** Во время звонка работают два
foreground-сервиса (доставка id 887 и микрофонный id 889) — оба ОБЯЗАНЫ показывать уведомление.
18.07 мы сделали микрофонный канал IMPORTANCE_MIN (чтобы не бросалось в глаза), 19.07 обезличили его
текст до «Orpheus / In call» (утечка префикса ключа). В сумме два уведомления стали БУКВАЛЬНО
неразличимы: одинаковый заголовок, текст и иконка, разные только id. Тихость проблему не решала.
Решение: оба сервиса держат ОДНО уведомление — микрофонный переиспользует id 887 и канал сервиса
доставки, а при остановке делает `STOP_FOREGROUND_DETACH` вместо `REMOVE` (иначе снёс бы то, что
постоянный сервис обязан держать дальше). В `stopCallService` добавлен идемпотентный `start()` —
страховка на случай, если сервис доставки в этот момент мёртв и отцеплённое уведомление зависнет.

**Попутная утечка, найденная там же:** обезличивание 19.07 закрыло показ префикса ключа только на
МИКРОФОННОМ уведомлении, а уведомление сервиса доставки продолжало получать `_displayName`, который
до разрешения контакта равен `contactPublicKey.substring(0, 8)`. То есть в шторке светилось
«e_wfH1Zr / 00:07». Добавлен флаг `_nameResolved`: имя уходит в шторку только после того, как
контакт реально найден в БД.

**Эхо (первые ~5 с) — причину установить нельзя,** и это честный вывод, а не отговорка: эхо
рождалось на телефоне собеседницы, а её лог звонка отсутствует (прислана выгрузка из ПАМЯТИ,
начинается в 11:11:54, инцидент не покрывает). Самая экономная версия, совместимая с таймингом, —
штатная сходимость аппаратного подавителя эха на дальнем конце. Версия «переключение ICE с TURN на
прямую пару в 11:04:47» проверена и отклонена: транспорт не меняет акустику.

**Урок для чеклиста:** «Поделиться» на экране отладки отдаёт ФАЙЛ (переживает перезапуск), а
копирование текста — только буфер в памяти. Тестировщикам просить именно файл.

**Статус:** analyze 566, 0 errors; test 436 passed.

---

## 2026-07-27 — Вес APK: 125.9 -> 101.4 МБ (b49) [wl/dev]

Задача ставилась как «ассеты», но настоящая находка оказалась в другом месте, а исходная
посылка — неверной. Фиксирую оба факта, чтобы не повторять.

**Ассеты дали 132 КБ, а не 5.2 МБ.** Посылка была взята из январского PR #1 («директория
`assets/images/` тащит в APK служебные наборы иконок android/ и ios/») и оказалась НЕВЕРНОЙ:
объявление директории во Flutter **не рекурсивно**, подпапки в бандл не попадали никогда —
проверено распаковкой APK b48, внутри лежали только три файла из корня. Реальная экономия —
`logo.png` (исходник для flutter_launcher_icons и flutter_native_splash, они читают его С ДИСКА
на сборке) и `logo.svg` (в коде не используется). Перевёл список на пофайловый, оставил только
`logo_mark.png`; в pubspec написал, ПОЧЕМУ это не 5 МБ, чтобы никто не «починил обратно».

**Настоящий вес — нативные библиотеки: 119 МБ из 126.** Лежали в трёх копиях:
x86_64 47 МБ, arm64-v8a 41 МБ, armeabi-v7a 32 МБ. x86_64 нужен ТОЛЬКО эмуляторам.
Сборка с `--target-platform android-arm,android-arm64` = **101.4 МБ, минус 24.6 МБ (19%)**;
меньше «сырых» 47 МБ, потому что в архиве библиотеки сжимаются. Реальные телефоны не затронуты:
обе ARM-архитектуры на месте.

**НЕ ПОЛУЧИЛОСЬ вынести в gradle (важно, не повторять).** Владелец выбрал вариант «пропиши в
gradle, чтобы флаг не забывали». Попробовал `ndk { abiFilters }` в `buildTypes.release` — размер
не изменился НИ НА БАЙТ; затем в `defaultConfig` — тот же результат, x86_64 остался в APK
(проверено распаковкой). Причина: библиотеки движка и плагинов раскладывает сам инструмент
Flutter по `--target-platform`, gradle-фильтр до них не дотягивается. Обе пробы откачены —
мёртвая настройка хуже отсутствующей, следующий решит, что фильтр работает. Вместо этого флаг
внесён в релизный чеклист CLAUDE.md (фаза 6) с объяснением, почему в gradle нельзя.

---

## 2026-07-27 — DEVICE-ТЕСТ b48 (Samsung): duress-трек ЗАЧЁТ [wl/dev]

Владелец прогнал чеклист на b48 (Samsung R5CY108JSXN, Android 16, обновление поверх b44 —
подпись совпала, данные сохранились): **всё работает**. Закрывает главный сценарий (открытый чат
не отдаётся по коду принуждения), экран «Безопасность» без действий, требующих текущий PIN,
комнаты и чат с разработчиком пустые без красной плашки, возврат настоящим PIN отдаёт всё реальное.

**Диагностическое ограничение, созданное сознательно:** вход по коду принуждения теперь НЕОТЛИЧИМ
в логе от обычного (строка одинакова — экран логов достижим из duress-сессии, и «Duress code
entered» было бы самой разрушительной подсказкой). Подтверждать включение режима можно только по
экрану. При разборе инцидентов это надо помнить.

**Шум в логе, который НЕ является поломкой:** `FlutterSecureStorage: EncryptedSharedPreferences
initialization failed. Falling back to custom ciphers` + `SecurityException: Could not decrypt key`.
Это ожидаемый путь: мы намеренно не используем EncryptedSharedPreferences, а пинуем PKCS1 +
AES-GCM (`secure_storage_options.dart:24` и разбор бага OAEP/MGF1 на строгом KeyMint там же).
«Custom ciphers» = наш сконфигурированный путь. Подтверждение целостности рядом в логе:
`WS: PoP ok — соединение установлено`, т.е. ключи прочитаны и личность на месте.

**НЕ проверено на устройстве (требует отдельной обвязки):** код удаления с локскрина (реально
стирает аккаунт — нужен тестовый, который не жалко) и «сообщение, пришедшее во время duress,
доезжает после входа настоящим PIN» (нужен второй телефон-отправитель).

---

## 2026-07-26 — DEVICE-ТЕСТ b47: найдено противоречие «текущего PIN» в duress [wl/dev]

**Первый же шаг device-теста дал находку, которую не поймал ни один из прогонов агентов.**
Владелец: залочил открытый чат → вошёл кодом принуждения → пустой профиль (главный фикс работает) →
пошёл в «Безопасность» → тапнул «Установить код принуждения» → приложение просит ввести PIN. А
«текущим PIN» для наблюдателя является ровно код принуждения, которым только что открыли приложение.
Он вводит его и получает «неверный PIN» (`pin_setup_screen.dart:290` требует
`PinVerifyResult.success`, duress-код даёт `PinVerifyResult.duress`).

**Почему это важно:** это ДОКАЗУЕМОЕ противоречие — код открывает приложение и тут же объявляется
неверным. У наблюдателя два объяснения, и вдумчивый выберет «мне дали подставной код». Проверяется
за десять секунд.

**Честно про происхождение:** кнопки «Изменить PIN» и «Отключить PIN» вели себя так ВСЕГДА (их я не
маскировал), но моя маскировка 908771d добавила третий такой вход — «Установить код принуждения».
Размен был в плюс (до него секция прямым текстом писала «Код принуждения установлен»), но работа
осталась недоделанной.

**Принцип решения (согласован с владельцем):** убирать ДОКАЗУЕМЫЕ противоречия, терпеть
НЕДОКАЗУЕМЫЕ странности. Отсутствие кнопки наблюдателю сравнить не с чем; PIN, который открывает
приложение и объявляется неверным, — проверяемый факт.

Отвергнуто по дороге: (а) принимать duress-код как «текущий PIN» с иллюзией применения — наблюдатель
«сменит PIN», при следующей блокировке новый код не подойдёт, и это тель ГРОМЧЕ исходного, плюс
можно словить авто-wipe; (б) «duress как честная вторая личность со своим PIN» — тянет опасный
хвост: что делает «Отключить PIN» в такой сессии? отключит код принуждения, и следующий запуск
откроет настоящий профиль без PIN вообще.

**Сделано:** под duress с экрана «Безопасность» убраны «Изменить PIN», «Отключить PIN» и ОБЕ секции
кодов целиком. Секции скрыты, а не показаны пустыми: пустая секция сама рассказывает наблюдателю про
существование режима принуждения — понятия, о котором ему знать незачем. Прежняя маскировка
`isDuressEnabled`/`isWipeCodeEnabled` стала мёртвой логикой и упрощена. Тест переписан на новое
поведение и заодно проверяет, что экран не превратился в обрубок (раздел «Защита от подбора» на
месте). Проверено попутно: ввод duress-кода в эту форму НЕ крутит счётчик неудачных попыток
(ветка duress зовёт `_resetFailedAttempts`), так что авто-wipe от этого не случится.

**Статус:** analyze 566, 0 errors; test 436 passed.

---

## 2026-07-26 — Duress закрыт полностью: потеря входящих, уведомления, confirmMainPin [wl/dev]

Последние три пункта списка. Сборка снова пересобрана (b47) по той же причине, что b46 сменила
b45: задание тестировщику содержит проверки, которых в предыдущем APK нет.

**1. Входящее личное сообщение в duress терялось НАВСЕГДА (тот же класс, что «лифт», но со
стороны получателя).** Строгий mutual-add спрашивает `isContact`, а под duress тот отвечает
`false` → кадр дропался ДО записи (incoming_message_handler.dart:128), при том что сервер уже
считал его доставленным и в оффлайн-очередь не клал. Фикс: `parkPersonalChatUntilRealSession`
в точке диспетчеризации WS кладёт конверт в `PendingInboxStorage` — ту же очередь, куда пишет
push-изолят при убитом приложении. `_drainPendingInbox` под duress не запускается (main.dart:610)
и уже останавливается, если duress включился посреди слива (:621), поэтому очередь разбирается
после входа НАСТОЯЩИМ PIN; дедуп по `message_id` гасит повтор. Откладываем ТОЛЬКО `type == 'chat'`:
комнаты и поддержка живут на сервере и перечитаются, а парковать `call-offer` бессмысленно —
воспроизводить протухший звонок нельзя. Функция сделана публичной ради теста (прецедент —
`dropRoutesAboveHome`).

**2. Уведомления под duress.** Тела и так обезличены (`l10n.newMessage`), но САМ ФАКТ уведомления
на пустом профиле доказывает наличие скрытого аккаунта. Гейт поставлен на границе, где main знает
про duress: `_IncomingNotificationsAdapter.showMessageNotification` (закрывает и личные, и
support-reply — он идёт через тот же адаптер) и `_handleRoomEventForBadge` (комнаты). Состояние
точки непрочитанного у комнат СОХРАНЯЕТСЯ — она невидима под duress и корректна после возврата.

**3. `verifyPin` внутри сессии снимал duress-гейт.** Добавлен `AuthService.confirmMainPin`:
проверяет основной PIN через тот же `verifyPin` (значит вся логика счётчика попыток и локаута
сохраняется), но восстанавливает флаг duress, если сессия была duress. Использован в
`_verifyWithAppPin` (экспорт seed). Сценарий, который это закрывает: жертву заставили подтвердить
ОДНО действие основным PIN — раньше это молча выключало гейт БД, и дальше наблюдатель видел
реальные данные на «пустом» профиле.

**Статус:** analyze 566, 0 errors / 0 warnings; test **436 passed**. Документация обновлена в тот
же заход (CLAUDE.md, SECURITY_REVIEW, FEATURES_AND_LIMITATIONS, DEVICE_TEST_CHECKLIST, ai_kb,
задание тестировщику) — урок из прошлого коммита учтён.

**Осталось по duress ровно одно:** push-изолят к режиму слеп (статики пер-изолятны, свой
`AuthService` с `_isDuressMode == false`). Сегодня безвредно: изолят работает только когда main
мёртв, а duress живёт лишь в живой сессии. Записано как P2.

---

## 2026-07-26 — Тест-сборка 1.1.8+46 (замена b45) [wl/dev]

b45 собрана ДО фикса кода удаления, а задание тестировщику уже содержит его проверку (пункт 0.9) —
на b45 она бы провалилась и выглядела как баг. Поэтому бамп до +46 и пересборка; в папке
тестировщика APK заменён, задание переименовано на b46. b45 никуда не деплоился, так что путаницы
у тестировщиков не будет.

---

## 2026-07-26 — Код удаления с локскрина заработал (не работал НИКОГДА) + хвост duress [wl/dev]

**Дефект:** `_showWipeConfirmDialog` (lock_screen.dart) звал `showDialog` из виджета, который в проде
живёт оверлеем в `MaterialApp.builder`. SDK создаёт Navigator в `routing` и лишь ПОТОМ заворачивает
его в `builder` (app.dart:1693-1727) — значит у контекста внутри builder Navigator НЕ предок,
`Navigator.of` бросает `FlutterError`, и `widget.onWipe(WipeReason.wipeCode)` недостижим с самого
появления фичи. Авто-wipe и panic-жест при этом работали.

**Метод:** сначала ТЕСТ, воспроизводящий баг. Новая группа в lock_screen_test.dart —
«LockScreen как оверлей (структура прода)»: лок поднимается в `builder`, а не в `home:`. До правки
два теста падали ровно с «Navigator operation requested with a context that does not include a
Navigator», третий (авто-wipe) проходил — то есть тест различает ветки, а не просто краснеет.
Старая группа с `home:` дефект не ловила В ПРИНЦИПЕ — вот почему он прожил так долго.

**Фикс:** подтверждение рисуется СЛОЕМ внутри самого LockScreen — поле `Completer<bool> _wipeConfirm`,
`Positioned.fill` последним элементом его `Stack`, результат отдаётся колбэком `onResult`, а не
`Navigator.pop`. В `dispose` висящий completer завершается `false`. Вариант «через navigatorKey»
отвергнут осознанно: диалог уехал бы в root Navigator, то есть ПОД непрозрачный оверлей лока —
невидимый и некликабельный, стало бы только тише.

**Побочная правка ради тестируемости:** `_HoldToWipeDialog` мерил удержание через `DateTime.now()`,
а в виджет-тестах стенные часы стоят — путь «удержал 2 с → стёрло» был непроверяем в принципе.
Перевёл на счётчик тиков таймера (50 мс). В проде разница в пределах миллисекунд, а под нагрузкой
тик приходит позже, то есть удержание становится чуть длиннее — ошибка в безопасную сторону.

**Хвост того же дня:** секретные 5 тапов не открывают экран логов под duress (там RAM-лог реальной
сессии + шаринг файла с диска); `clearChatHistory` получил duress-гейт — он был единственным путём
удаления без него и через удержанный чат сносил реальную переписку безвозвратно.

**Документация:** пришлось пройти второй раз по всем местам, где час назад я задокументировал
«wipe-код не работает никогда» — CLAUDE.md, ARCHITECTURE, SECURITY_REVIEW (P0 → ИСПРАВЛЕНО),
FEATURES_AND_LIMITATIONS, PROJECT_STRUCTURE, PROJECT_OVERVIEW, PHILOSOPHY, ADR-0003,
DEVICE_TEST_CHECKLIST, TESTER_ASSIGNMENT, SECURE_STORAGE_V10_CHECKLIST, ai_kb/08 и задание
тестировщику. Урок: правка «известных дефектов» в документации живёт ровно до следующего коммита —
чинить дефект и обновлять его описание надо одним заходом.

**Статус:** analyze 566, 0 errors; test **431 passed** (403 в начале дня).

---

## 2026-07-26 — Синхронизация документации после duress-трека [wl/dev]

Разбор 4 читателями (103 кандидата → 64 верифицированных правки) + применение по группам файлов.
Затронуто 22 файла: CLAUDE.md (правил сам — по ней ориентируются агенты), docs/ARCHITECTURE,
PROJECT_STRUCTURE, FUNCTIONAL_PRINCIPLES, DEVELOPMENT_GUIDE, PROJECT_OVERVIEW, SECURITY_REVIEW,
FEATURES_AND_LIMITATIONS, PHILOSOPHY, DECISIONS/0003-security-system, DEVICE_TEST_CHECKLIST,
TESTER_ASSIGNMENT, PUBLIC_CHANGELOG_RU, SECURE_STORAGE_V10_CHECKLIST, 9 файлов docs/ai_kb, плюс
gitignored пакет `Колобанов/` (задание тестировщику под b45).

**Две поправки к моим же предыдущим утверждениям, обе подтверждены по коду:**
1. `PanicWipeService` РАБОТАЕТ: `init()` зовётся в main.dart:358, обсервер регистрируется
   (panic_wipe_service.dart:70), стирание идёт через `_auth.performWipe()`. Мёртв только
   необязательный колбэк `onPanicWipe`. Раньше я записал сервис в мёртвый код — неверно.
   Panic-жест теперь везде описан как РАБОЧИЙ путь стирания рядом с авто-wipe.
2. Уведомления в duress НЕ раскрывают содержимое: `body: l10n.newMessage`
   (notification_service.dart:645), для комнат `l10n.newMessageInChat` (:677), имя комнаты
   игнорируется намеренно (:662-666). Утечка — только сам ФАКТ уведомления. Раньше сказал
   «с реальным содержимым» — хуже реальности.

**Своя находка при ревю правок (важно):** база знаний Оракула грузится НА СЕРВЕРЕ из `docs/ai_kb/`
(ai_assistant_service.dart:45), а сам Оракул в списке контактов НЕ гейтится по duress
(`showOracle = !searching`, contacts_screen.dart:298). То есть под принуждением наблюдатель может
спросить Оракула, и тот ответит по этим файлам. Общее описание duress там оставил (это публичная
информация о продукте, безопасность режима не держится на незнании фичи), но убрал строку-
РАСПОЗНАВАЛКУ «переключатель отскакивает назад — это ожидаемое поведение»: она объясняла
наблюдателю, как понять, что сессия duress. Инвариант на будущее: в `docs/ai_kb/` нельзя писать
признаки, по которым duress-сессию можно отличить от обычной.

**Попутно исправлено устаревшее (вне duress-темы):** docs/ARCHITECTURE.md в трёх местах описывал
FCM, хотя Google из проекта выпилен (README синхронизировали 02.07, этот файл пропустили) —
уведомления, телеметрия фонового обработчика и § 4. При правке § 4 я сам сначала написал, что
уведомления поднимаются одним кодом из обоих изолятов — проверил и переписал: push-изолят держит
только своё уведомление сервиса (id 887), уведомления о сообщениях поднимает main-изолят через
`IncomingMessageHandler` → `NotificationService.showMessageNotification`.

`docs/PUBLIC_CHANGELOG_RU.md` получил блок `1.1.8 (LATEST)` — это готовый текст для регистрации
версии в админке.

---

## 2026-07-26 — Тест-сборка 1.1.8+45 [wl/dev]

Бамп `pubspec.yaml` 1.1.8+44 → +45. `AppConfig.appVersion` не трогаем — version name тот же
(`v1.1.8`), меняется только build-номер. `debugFileLogging` остаётся `true` (тест-сборка: файловые
логи + скрины разрешены; перед боевым релизом — `false`).

В сборку входит: фикс залипшего презенса (414fdf3, был готов ещё 24.07 и в b44 не попал) + весь
duress-трек этого дня (259f9bd, 98d3f9c, 908771d, 958d08b). Device-чеклист — в следующем разделе
про duress, плюс остаток по outbox: удалить недоставленное сообщение с часиками и убедиться, что
оно НЕ дошло получателю после реконнекта (пути удаления чистят outbox: database_service.dart:1092,
:1111, :1070). «Ровно одна копия у получателя» владелец проверил 26.07 — зачёт.

---

## 2026-07-26 — Duress: вход по коду принуждения больше не отдаёт UI реальной сессии [wl/dev]

**Контекст:** дефект был замечен «вне диффа» при разборе outbox 24.07 и отложен как отдельный.
Лок в приложении — ОВЕРЛЕЙ в `MaterialApp.builder` поверх Navigator'а (сознательное решение,
см. секцию Security в CHANGELOG: как `home` он перекрывался запушенными маршрутами и PIN
обходился). Следствие, которое тогда не учли: сами запушенные маршруты остаются
смонтированными вместе со State и уже расшифрованными данными, а `_onDuressMode` снимал
только флаг лока. Разбор 26.07 (22 агента: 6 карт поверхности + адверсариальная проверка
каждой находки + 3 независимых дизайна + синтез): 59 кандидатов, 12 проверено адверсариально,
**0 опровергнуто**; после дедупа 6 разных дефектов, стек Navigator'а — лишь один из них.

**Сделано (этот коммит):** `PushedRouteTracker` (NavigatorObserver, регистрируется рядом с
`navigatorKey`; список обсерверов — top-level final, иначе `didUpdateWidget` отвязывает
обсервер на каждом ребилде) + `dropRoutesAboveHome()` на `NavigatorState.removeRoute` —
синхронно и БЕЗ анимации выхода, пока непрозрачный лок ещё нарисован. `popUntil` (прецедент
`settings_screen.dart:233`) НЕ подходит: маршрут логически снят, но доигрывает ~300мс уже
после исчезновения лока — проверено мутацией, покадровый тест падает на КАДРЕ 0. Сброс в трёх
точках: вход по duress, `onWipeCompleted` (наконец выполняет обещание комментария
в `performWipe`: «чтобы навигация сбросилась»), и настоящий PIN ПОСЛЕ duress-сессии
(флаг `_duressUiActive` в `_MyAppState`, т.к. `lock()` сам сбрасывает duress в AuthService и
спросить его потом нельзя). Плюс: реальный pending/ringing звонок не переезжает в duress
(`_pendingCall = null` + `PendingCallStorage.clear()` + `endAllCalls`), обе resumed-ветки
гейтятся на `isDuressMode`; строка лога о входе в duress больше не называет режим (журнал
доступен из этой же сессии секретными тапами) — теперь дословно как в success-ветке; два
`mounted`-гарда там, где `removeRoute` завершает future маршрута на уже размонтированном State
(`rooms_screen._refreshRooms`, `settings_screen` onSecurity).

**Сознательно НЕ взято из плана:** `DebugLogger.clear()` при входе в duress. Проверено:
экран логов показывает RAM, но кнопка «поделиться» отдаёт ФАЙЛ, а `clear()` чистит только
список в памяти — полумера, которая вдобавок сжигает диагностику device-тестов. Правильное
лечение — закрыть вход на экран логов в duress, идёт отдельно.

**НЕ закрыто этим коммитом (подтверждено адверсариально, отдельные коммиты этой же версии):**
F1 комнаты не гейтятся (`RoomsService._pubkey` отдаёт настоящий pubkey → сервер заново
присылает реальные комнаты, превью и историю; можно писать/удалять от лица жертвы; лечится
одной строкой — все 11 методов уже гейтятся на `_pubkey == null`); F2 чат с разработчиком то
же самое (в `loadMessages` нужна duress-ветка с ПУСТЫМ результатом и `_error == null`, иначе
текст ошибки сам станет tell'ом); F3 экран «Безопасность» прямым текстом сообщает, что код
принуждения настроен (маскировать локальными флагами, секции НЕ скрывать — их отсутствие тоже
tell); F4 «Экспорт аккаунта» в duress отдаёт root seed по системному коду устройства, который
у наблюдателя обычно есть (требовать `_verifyWithAppPin`); F5 пять сеттеров конфига в duress
персистентно меняют реальные настройки (гейт в КАЖДЫЙ сеттер, не в `_saveConfig` — иначе
сломается `disableDuressCode`). Плюс отдельно найденное: `clearChatHistory` — единственный
путь удаления БЕЗ duress-гейта (через удержанный чат сносил реальные сообщения безвозвратно;
сам стек теперь сбрасывается, но гейт всё равно нужен), код удаления с локскрина не доходит
до `performWipe` (showDialog из виджета без Navigator в предках), `PanicWipeService.init()` не
вызывается нигде, входящие во время duress дропаются НАВСЕГДА, push-изолят слеп к duress.

**Статус:** analyze 567 issues — ровно как до правки, 0 errors / 0 warnings; test **412 passed**
(было 403): 8 widget-тестов вокруг РЕАЛЬНОГО ChatScreen (включая покадровый) + тест гигиены
лога. Мутационная проверка: без фикса падают 6 из 8, с `popUntil` — покадровый. Далее: device-
чеклист (чат/экспорт/заметки под принуждением; настоящий PIN по-прежнему возвращает в чат;
звонок через локскрин не сломан) и F1-F5.

**F1+F2+F6 (следующий коммит, 26.07):** `RoomsService._pubkey` и `SupportChatService._pubkey`
возвращают null в duress — данные комнат и поддержки лежат на СЕРВЕРЕ, гейт БД их не касался.
Все 11 методов RoomsService уже гейтились на `_pubkey == null`: чтения → пусто, prefs → безопасные
дефолты, мутации → throw, а UI ловит всё в `l10n.connectionError` — от недоступного сервера
неотличимо, нового tell'а нет (проверено по rooms_screen.dart:97,126). В support пришлось добавить
ОТДЕЛЬНУЮ duress-ветку в `loadMessages` ДО проверки на null: иначе сработала бы ветка
`_error = 'Account not initialized'`, а экран рисует ошибку красной плашкой с текстом
(support_chat_screen.dart:226) — это сам по себе tell; ветка зовёт `clear()`, что заодно вычищает
кеш синглтона (сообщения реальной сессии оставались в `_messages` и были видны свежему экрану в
duress). `handleIncomingReply` — no-op в duress (живой ответ по WS не всплывает). Точка «Комнаты»
на таб-баре гасится в home_screen (НЕ мутацией RoomUnreadService: иначе после настоящей
разблокировки индикатор остался бы погашенным до следующего syncWithRooms). Проверено, что
`syncWithRooms([])` в duress НЕ портит состояние реальной сессии: `_lastSeen` непустой → firstRun
false → `changed` false → до `_persist()` дело не доходит. Тестовый шов: `AuthService.debugSetDuressMode`
(@visibleForTesting — прод читает флаг с синглтона, а у него secure storage это плагин, недоступный
в unit-тестах) + существующий `CryptoService.deriveFromSeedForTest` для настоящего адреса. 9 новых
тестов, каждая группа с КОНТРОЛЬНЫМ тестом «вне duress запрос уходит и несёт настоящий X-Pubkey» —
иначе тест был бы пустым (без адреса `_pubkey` null и так). Побочно замечено: старые тесты
rooms_service_test обёрнуты в `if (rooms.isNotEmpty)` и проходили вакуумно (адреса в тестах нет) —
не трогал, но HTTP-путь там де-факто не покрыт.

**F3+F4+F5 (третий коммит, 26.07):** F3 — `security_settings_screen.build` считает
`duressActive = _auth.isDuressMode` и гасит `isDuressEnabled`/`isWipeCodeEnabled`; разделы
ОСТАЮТСЯ видимыми в состоянии «не настроено» (исчезновение = tell), а ветка настройки ведёт в
`PinSetupScreen`, где `_processSetDuress` шаг 0 требует `PinVerifyResult.success` — перезаписать
код принуждения из duress нельзя (проверено по коду, не на слово). F4 — в `_exportAccount`
добавлен `requireAppPin = authService.isDuressMode`, и `systemAuthAvailable` считается через
`!requireAppPin && (...)`: короткое замыкание, поэтому вне duress НИ ОДИН вызов плагина не
меняется. Мотив: гейт был `biometricOnly: false`, т.е. принимал КОД УСТРОЙСТВА, который у
наблюдателя обычно есть, а экспорт отдаёт 32-байтный root seed. F5 — гейт `if (_isDuressMode) return;`
в каждом из пяти сеттеров (setAutoWipe / setPanicGestureEnabled / setBiometricEnabled /
setInactivityLockSeconds / setMessageRetention), НЕ в `_saveConfig`.

**Осознанный компромисс F5 (решение за владельцем):** экран перечитывает конфиг после сеттера,
поэтому в duress переключатель ВИЗУАЛЬНО отскакивает назад — мелкий tell. Иллюзию «применилось»
не делаю: она требует либо локального состояния экрана (умрёт при переоткрытии, т.е. tell
всё равно), либо правки конфига в памяти — а она протечёт в настоящую сессию, потому что
`verifyPin` конфиг из storage не перечитывает. Выбрал fail-closed: настройки жертвы целы.

**Новая находка по ходу F4:** `verifyPin(основной PIN)` внутри duress-сессии вызывает
`_setDuressMode(false)` — то есть ЛЮБАЯ внутрисессионная проверка основного PIN (например
`_verifyWithAppPin` в экспорте) молча снимает гейт БД, оставляя UI duress-сессии. Новой дыры не
создаёт (у кого есть основной PIN, тот и так входит нормально), но состояние становится
рассогласованным: пустой профиль на экране + разгейченная база под ним. Это тот же корень, что
мёртвый `exitDuressMode`: внутрисессионный выход существует, но НЕ задуман и не приводит UI в
порядок. Отдельным пунктом.

**Статус F1-F5:** analyze 567 — как в базе, 0 errors; test **425 passed** (403 до всей работы:
+9 стек/лог, +9 комнаты/поддержка, +4 экран «Безопасность»/сеттеры). У каждой duress-группы есть КОНТРОЛЬНЫЙ тест «вне duress работает как раньше», иначе
проверки проходили бы вакуумно. F4 виджет-тестом НЕ покрыт сознательно: `LocalAuthentication`
...
`AlexWhiplash/orpheus_client` — наш, мержим и деплоим сами.

---

## 2026-07-24 — Персистентный outbox: сообщение не теряется в умирающей сети (инцидент «лифт») [wl/dev]

**Контекст:** 23.07 тестировщик отправил сообщение из лифта — у него «отправлено» (галка),
получателю не пришло никогда. Разбор (33 агента, клиент+сервер): отправка chat была
fire-and-forget в полумёртвый TCP (статус ещё Connected до ~20с нативного ping-watchdog),
server-ack для chat не существовал, статус sent ставился безусловно. 11 подтверждённых окон
потери; главное серверное — half-open swallow получателя (закроем этапом C: chat-recv).
Перед реализацией — обязательная переверификация по коду (5 агентов, 53 сюрприза, все учтены).

**Клиент (этот коммит):** таблица `outbox` в SQLCipher (миграция v9→v10, аддитивная +
разовый импорт prefs-очереди PendingActionsService); `sendChatMessage` всегда через outbox
(персист ДО сокета), удаление ТОЛЬКО по подтверждению; подтверждение двухрежимное — chat-ack
по `caps` из pop-ok, иначе pong-fence (pong подтверждает всё записанное до его ping — сервер
читает кадры последовательно, контракт закреплён серверным `tests/ws_helpers.py`); FIFO-учёт
пингов/понгов по поколению сокета; pong-watchdog ~10с ТОЛЬКО при in-flight (батарея: в покое
не тикает, новых кадров не добавляет); ретрай 30с при Connected и непустой очереди, кап 8
попыток → честный failed; UI: «часики» до подтверждения, failed всегда виден («Не отправлено»,
не прячется в склейке) + «Повторить отправку» (тот же messageId — дедуп получателя гасит
дубли); reconcile на старт/анлок/резюм (сироты sending→failed, гейт по локу/duress); duress:
очередь реального профиля не читается и не пополняется; счётчик «Очередь» статус-экрана
переведён на outbox; весь путь отправки логируется (разбор инцидента был слеп — успешный путь
не писал в файловый лог ни строки). l10n: `notSent`, `resendMessage` (EN/RU). CLAUDE.md: БД v10.

**Сервер (ветка wl/chat-ack, 3 коммита, НЕ мержено в wl/dev):** chat-ack {message_id, queued}
в сокет-источник + caps в pop-ok + колонка message_id и частичный уникальный индекс в
offline_messages (дедуп ресендов, raw ON CONFLICT DO NOTHING) + фикс TypeError send_push
(навсегда убивал redis-listener) + phantom-delivery (receivers>1). 6 новых pytest-тестов.

**Статус:** клиент analyze 0 errors / 0 warnings в затронутых файлах; test 402 passed.
Сервер pytest весь зелёный локально (venv py3.12). Далее: адверсариальное ревью диффов,
device-тест «лифта» (silent drop на роутере; airplane-mode НЕ воспроизводит окно).

---

## 2026-07-24 — Device-тест b44 (Samsung) + фикс залипшего презенса [wl/dev]

Device-тест «лифта» на Samsung (владелец вёл UI): миграция v9→v10 чисто на живых данных;
silent drop → часики; смерть сокета замечена за ~10-20с (logcat: onDone 16:18:13); connect-
watchdog корректно бросает зависшие попытки (фикс b8f217e виден в деле); восстановление
сети → PoP ok за ~2с → автодосылка → галка. Kill-тест: часики → смахнул приложение →
вернул сеть → открыл → ушло. Инцидент «лифт» подтверждён закрытым на устройстве.

Pixel 7 Pro для тестов клиента непригоден: на нём OrpheusOS (свой ROM, Android 17), клиент
вшит как system app b40 — adb install падает fs-verity (user-build, enforcing); самоапдейт
под подозрением; разбор передан в OrpheusOS-трек отдельным промтом.

Попутная находка теста: контакт «онлайн» залипал при обрыве СВОЕЙ сети (PresenceService
сбрасывал состояние только… никогда — статус-листенер обрабатывал лишь Connected).
Фикс: на Disconnected/AuthFailed презенс сбрасывается (без соединения статус неизвестен),
после реконнекта возвращается полной переподпиской. +1 unit-тест (6 passed).

---

## 2026-07-24 — Бамп версии 1.1.8+44 (тест-сборка: outbox + chat-ack) [wl/dev]

Версия поднята до `1.1.8+44` (name «1.1.8», versionCode 44 поверх b43; config.dart не
трогался, debugFileLogging=true). Содержимое: персистентный outbox с подтверждением
сервера (78fbd44) + hardening по ревью (b8f217e). Сервер с caps=['chat-ack'] уже
задеплоен (подтверждено probe-ом) — сборка сразу работает в строгом ack-режиме.
Проверка на устройстве: лог `PoP ok — соединение установлено! (подтверждение chat:
chat-ack)`; device-тест «лифта» — silent drop на роутере (НЕ airplane-mode).

---

## 2026-07-24 — Хардening по адверсариальному ревью outbox (23 находки, 6 линз) [wl/dev]

Ревью диффа 78fbd44 + серверной wl/chat-ack (6 линз; верификационные субагенты упали по
лимиту сессии — все находки разобраны вручную по коду). Исправлено клиентски:
- fence-ping больше не может уйти в сокет другого поколения (гейт gen/статуса в
  _sendFencePing; между записью пачки и пингом есть await);
- watchdog не продлевается новыми fence-пингами (активная переписка не откладывала бы
  детекцию мёртвого сокета); pong-прогресс перевзводит как раньше;
- _failOutboxMessage «захватывает» строку удалением (removeFromOutbox теперь возвращает
  count) — гонка с параллельным chat-ack не красит доставленное в failed;
- кап размера проверяется и в drain (импорт legacy prefs-очереди шёл мимо sendChatMessage);
- reconcile: гейт «нет аккаунта — не трогаем БД» (после wipe database-getter воссоздавал
  пустую БД + НОВЫЙ ключ в secure storage — урок «мины миграции»); возрастной фильтр 2 мин
  (гонка с живой отправкой в окне resolveEncKey/encrypt); NULL-safe NOT IN + NOT NULL в
  схеме outbox;
- удаление сообщений/чата/контакта чистит и outbox (удалённое не дошлётся после реконнекта);
- «Повторить отправку» скрыт для записей звонков (messageId=call_id совпадает с локальной
  записью, текст у неё — наше направление, ресенд слал бы перевёрнутое);
- sending не прячется в склейке минут (часики видны всегда); catch-блоки отправки не
  роняют второе исключение при wipe;
- _handleDisconnect обнуляет мёртвый _channel — чинит пре-существующий вечный no-op
  8-секундного connect-watchdog на реконнектах (зависший connect не отбрасывался).

Сервер (wl/chat-ack): нода-получатель без локального клиента больше не дропает форвардное
сообщение молча (save_offline вместо continue — иначе ack, выданный нодой-отправителем,
врал бы); обработка pubsub-события обёрнута per-message (одно проблемное сообщение не
убивает listener, мёртвый listener чужой ноды = фантомные receivers).

Принято как задокументированные ограничения (в коде/отчёте): fence-режим против старого
сервера ложно подтверждает кадр, съеденный лицензионным гейтом (лечится деплоем chat-ack);
DoS-trim 500/пару может стереть chat с уже выданным ack (закроет этап C: TTL/store-until-ack);
двойной импорт legacy prefs-записей без messageId при крэше посреди импорта (разовый
переходный путь). Вне диффа, ТРЕБУЕТ отдельного разбора: duress-разблокировка не сбрасывает
Navigator-стек — удержанный ChatScreen реальной сессии виден duress-наблюдателю.

---

## 2026-07-19 — Бамп версии 1.1.8+40 (тест-сборка)

Версия поднята до `1.1.8+40` (name «1.1.8», versionCode 40 поверх b39; `config.dart` не
трогался). Содержимое: гейт сигналов звонка (c2e28b0) + иконка статусбара/честный регион
(a9eb700). Ставится поверх (боевой ключ).
...
Knox «Автоблокировка» (Auto Blocker, One UI 6+) запрещает установку из неавторизованных
источников = блокирует наш сайдлоад-апдейтер (после подтверждения). На его втором Samsung
(SM-S938B) она выключена → там ставится. Ключевое: **приложение НЕ может её определить,
обойти или запросить исключение** (Knox-уровень, выше REQUEST_INSTALL_PACKAGES; браузерный
фолбэк тоже сайдлоад → тоже блокируется). Единственный рычаг — пользователь отключает её сам.
Затрагивает только Samsung с включённой Автоблокировкой (на новых Galaxy может быть вкл. по
умолчанию); другие вендоры — штатный unknown-sources, уже закрыт. Решение владельца: A+B
(подсказать, обход невозможен).

**Сделано (`update_service.dart`):**
- `_isSamsungDevice()` — device_info_plus, кэш, строго `if (!Platform.isAndroid) return false`
  (иначе повис бы в тестах, как path_provider в b41).
- (A) `_reportPendingInstallFailure`: при отказе на Samsung (или явном следе `auto blocker`
  в message) — снекбар `updateRejectedSamsungBlocker` вместо generic; ABORTED(3) на Samsung
  больше не считается тихой отменой (Автоблокировка выглядит как abort), на не-Samsung —
  по-прежнему тихо. Точные причины (подпись/место/повреждён) приоритетнее. Сигнал
  Автоблокировки СУЖЕН до `auto blocker`/`autoblocker` (без общего `blocked` — иначе Play
  Protect на Pixel ложно подписался бы Samsung).
- (B) окно обновления: на Samsung под текстом — мелкая заметка `updateSamsungBlockerHint`;
  `isSamsung` считается до показа окна (на хосте false → тесты device_info не трогают).
- l10n `updateRejectedSamsungBlocker` + `updateSamsungBlockerHint` (EN+RU).

**Проверки:** `flutter analyze` — 0 ошибок (старые avoid_print/unnecessary_import не мои),
`flutter test` — 392 passed, апдейтер отдельно +8. Диагностика, root-cause обойти нельзя.
...
Samsung, 11:48). Разбор: собеседник прислал call-rejected → call-answer (+0.5с) → call-rejected
(дубль). Ответ после реджекта РЕАНИМИРОВАЛ WebRTC (state Rejected→Connecting); затем юзер
перезвонил, и ice-restart МЁРТВОГО звонка №1 лёг на PeerConnection звонка №2 → «Called in
wrong state: have-local-offer» → вечный «ICE restart…». Фильтр call_id стоял только на
hang-up/rejected. Почему собеседник прислал пару reject+answer — не выяснить (логи Pixel
перетёрлись); по решению владельца callee-сторону не трогаем, пока не воспроизведётся.

**Сделано (`call_screen.dart`):** единый гейт в подписке сигналов — (1) call_id: чужой/старый
идентификатор → игнор с логом, для ВСЕХ типов (answer/ice-candidate/ice-restart/
ice-restart-answer/hang-up/rejected; сигнал без call_id — легаси-пропуск); (2) тирдаун:
после _messagesSent поздние answer/ICE игнорируются (hang-up/rejected пропускаются —
_onRemoteHangup идемпотентен). Старый локальный фильтр в ветке hang-up/rejected удалён
как поглощённый.

---

## 2026-07-19 — b40: иконка сервиса из статусбара + честный регион

**Контекст (визуальные жалобы владельца, скрины):** (1) два щита в статусбаре — постоянное
уведомление сервиса доставки на канале Importance.low держало иконку всегда; (2) регион RU
при немецком VPN — кэш 12ч в prefs показывался с подписью «По IP», тап-диагностика владельца
подтвердила: цепочка и VPN-маршрутизация работают, виноват только кэш. Гео-модуль не писал
в логи вообще.

**Сделано:**
- `push_connection_service.dart`: канал v3 (`orpheus_connection_v3`, Importance.min) —
  иконки в статусбаре нет, уведомление в «тихих»; живучесть FGS не зависит от важности
  канала (обкатано на orpheus_call_audio_min в b39).
- `network_monitor_service.dart`: `lastNetworkChangeAt` — отметка ЛЮБОГО сдвига набора
  типов сети (setEquals; networkSwitch не годится — сравнивает только .first и не ловит
  VPN, который connectivity_plus дописывает не первым; семантику networkSwitch для WS
  не трогали).
- `geo_service.dart`: кэш не считается свежим, если записан до lastNetworkChangeAt
  (VPN вкл/выкл = смена внешнего IP); `GeoSource lastSource` (network/cache/none);
  логирование GEO: кэш+возраст, сеть+эндпоинт, сбой цепочки, офлайн.
- `status_screen.dart` + l10n (`regionSourceIpCache` EN/RU): кэш подписывается
  «По IP • кэш, тап — обновить» вместо маскировки под свежий «По IP».
- Ограничение (зафиксировано в комментарии): смену сети при неработающем приложении
  отследить нечем — там спасают TTL 12ч и тап.

**Коммиты:** гейт сигналов звонка — отдельным коммитом (c2e28b0), иконка+гео — этим.

**Проверки:** `flutter analyze` — 0 ошибок, `flutter test` — 392 passed, `flutter gen-l10n` ок.
Device-чеклист: VPN вкл → открыть «Систему» (без тапа!) → регион свежий/подпись честная;
статусбар без постоянного щита; звонок с быстрым reject+answer больше не завешивает
следующий набор (в логе — «Игнорирую … чужого звонка»).

---

## 2026-07-18 — b39: тап-гард входящего, обезличенное мик-уведомление, глушитель миграции

**Контекст:** device-тест звонков (скрины+лог, телефон MU5pv2 на b37). Репорт «что-то задвоилось».
Разбор лога показал ТРИ независимых факта, ни один не связан с презенс-деплоем:

1. «Задвоение» записей в чате (18:18 Исходящий + Пропущенный·Входящий) — два РЕАЛЬНЫХ звонка:
   входящий 18:18:58 был отклонён кнопочным путём через 330мс после показа экрана (маркер
   dispose-пути отсутствует) — тап «в полёте» при наборе текста попал в «Отклонить»; LiChan
   перезвонил через 4с, тот звонок состоялся. Дедуп записей работает корректно.
2. Два уведомления в звонке: id 887 (сервис доставки, «LiChan / 00:18») + id 889 (микрофонный
   `CallAudioService`, «e_wfH1Zr / In call») — в мик-уведомление уходил `_displayName` ДО
   резолва имени (= префикс ключа, утечка в шторку; call_screen.dart:146).
3. Вечный «Сбой миграции secure storage: Key mismatch after algorithm change» каждый холодный
   старт (наше логирование b36 поймало): хвост записей старого шифра (OAEP, до b34) валит
   deleteAll, флаг не выставляется, PKCS1-чтения при этом работают (Identity loaded ✓).

**Сделано (b39):**
- `call_screen.dart`: тап-гард 400мс для Incoming (`_ignoreEarlyIncomingTap` в `_acceptCall`/
  `_endCallButton`; autoAnswer=Connecting и watchdog=Dialing под гард не попадают);
  `startCallAudio()` без имени (нейтральное уведомление).
- `CallAudioService.kt`: канал v2 `orpheus_call_audio_min` (IMPORTANCE_MIN, каналы неизменяемы)
  + PRIORITY_MIN — мик-уведомление минимально заметно.
- `secure_storage_options.dart`: при «Key mismatch after algorithm change» на deleteAll и ЖИВОМ
  seed — флаг миграции выставляется, ретраи прекращаются (мёртвый OAEP-хвост безвреден).

**Проверки:** `flutter analyze` — 0 ошибок, `flutter test` — 392 passed. Device: входящий при
открытой клавиатуре не отклоняется мгновенным тапом; в шторке одно заметное уведомление звонка
без префикса ключа; на MU5pv2 после первого старта b39 «Сбой миграции» исчезает из логов.

---

## 2026-07-18 — Презенс: «онлайн» = приложение запущено (b38 + сервер PR #19)

**Контекст:** после деплоя #18 владелец заметил, что «онлайн» отражает «есть сокет», а не
активность. Read-only проверка (2 агента + свой разбор диффа cfbeb05..wl/dev) показала:
семантику никто не менял — онлайн всегда был «≥1 PoP-сокет» (is_first в promote()), а
push-сервис держит сокет и при убитом приложении → «онлайн» = «телефон включён». #18 лишь
починил залипание презенса и сделал это видимым. Владелец выбрал семантику
«приложение запущено» (свёрнутое = онлайн, убитое = оффлайн).

**Сделано (сервер, wl/dev 32115b7, PR #19 — мерж по отмашке):** роль сокета `?role=svc`
у push-изолята; сервисные сокеты — routing-таргеты, но вне презенса (online по первому
main, offline по смерти последнего main даже при живых сервисных; redis-ZSET освежают
только main; takeover сервисным сокетом, снявший последний мёртвый main, объявляет
offline). Немаркированные сокеты = main → старые клиенты (≤b37) без изменений.
Тесты: 3 новых кейса + `pop_connect(service=)`; сьют 185 passed. Грабля тестовой обвязки:
`receive_json_with_timeout` при таймауте оставляет зомби-поток, съедающий следующий фрейм —
паттерн «жду-таймаут-потом-читаю» заменён сентинелом (subscribe→state).

**Сделано (клиент, b38):** `webSocketUrl(..., service: true)` у push-изолята
(`config.dart` + `push_connection_service.dart`, 2 строки). `flutter analyze` — 0 ошибок,
392 passed.

**Device-чеклист (после мержа #19):** убить приложение на A → на B точка гаснет ≤1.5 мин;
свёрнутое A — онлайн; открыть A — онлайн сразу; сообщение/звонок в убитое A доходят;
b37 против нового сервера — как раньше.

---

## 2026-07-18 — Бамп версии 1.1.8+37 (тест-сборка)

Версия поднята до `1.1.8+37` (name «1.1.8» без изменений, versionCode 37 поверх b36; `config.dart`
не трогался). Содержимое: честные ошибки активации кода (запись ниже). Серверная половина фикса
ждёт мержа/деплоя wl/dev Вадимом — до этого перевзведённые коды всё ещё дают «Ошибка соединения»,
но теперь с диагнозом в файловом логе.

---

## 2026-07-18 — «Сломалась активация по коду»: корень серверный (UNIQUE tx_id), клиент — честные ошибки

**Контекст:** владелец на b36 получил «Ошибка соединения. Проверьте интернет» при активации кода
TEST1 (скрин). Подозрение на регрессию b35/b36 — НЕ подтвердилось: клиентские коммиты этот путь
не трогали.

**Диагноз (проверен по коду сервера + живыми пробами API):**
- `licenses.tx_id` UNIQUE (payments.py), а `activate-promo` пишет `tx_id=PROMO_<код>` навсегда.
  Перевзведённый в админке тест-код (`is_used=false`, строка licenses осталась) на повторной
  активации падает IntegrityError -> необработанный **plain-text** 500 (глобального
  exception-handler нет) -> клиентский `json.decode` кидает -> ложная «Ошибка соединения».
  Взведено серверным хардненингом 09.07 (unique tx_id); последняя успешная активация кода у
  владельца была раньше. Свежие (никогда не использованные) коды работают.
- Пробы прода: `activate-promo` с неверным кодом — валидный JSON за 0.3с; 429 rate-limit
  (10/5мин с IP) — тоже JSON `{"detail": ...}`, но клиент показывал его как «Неверный код».

**Сделано (сервер, vadimkolobanov/Orpheus ветка wl/dev, 60257db — деплой за Вадимом):**
- `activate-promo`: pre-check `licenses.tx_id` -> честный JSON «Код уже использован» вместо 500.
- Глобальный exception-handler: любой необработанный сбой -> JSON 500 + traceback в stdout.
- Регрессионный тест (активация -> перевзвод -> активация = JSON-ошибка, не 500); проверено,
  что без фикса тест падает. Полный сьют: 182 passed.

**Сделано (клиент, `license_screen.dart`):**
- `_activatePromo`: логирование причины сбоя в DebugLogger (HTTP-статус + тело до 200 символов
  при отказе; тип+текст исключения при сбое; сам код активации НЕ логируется);
  фолбэк на `detail` (формат FastAPI/429) при показе серверной ошибки (только String — у 422
  detail список); таймаут POST 15с (раньше зависший коннект держал спиннер минуты).

**Проверки:** `flutter analyze` — 0 ошибок, `flutter test` — 392 passed. Device: после деплоя
сервера повторная активация перевзведённого кода должна показать «Код уже использован», свежего —
пройти; до деплоя — прежняя «Ошибка соединения», но с диагнозом в файловом логе.

---

## 2026-07-17 — Бамп версии 1.1.8+36 (тест-сборка)

Версия поднята до `1.1.8+36` (name «1.1.8» без изменений, versionCode 36 монотонно поверх b35;
`config.dart` не трогался — `appVersion = v1.1.8` тот же). Содержимое сборки — два коммита
инцидента 17.07: миграционный флаг после wipe + защита БД. На устройстве стоит b35 — b36
ставится поверх (тот же релизный ключ) для device-проверки чеклиста wipe -> восстановление ->
холодный старт.

---

## 2026-07-17 — Защита БД: single-flight, запрет генерации ключа в push-изоляте, orphan вместо удаления

Второй коммит инцидента 17.07 (диагноз и детали — в записи ниже «Корень ночного слёта аккаунта»):
`database_service.dart` + entry-points изолятов. Файл БД при «отсутствующем» ключе откладывается
в `<db>.orphaned` (не удаляется), одновременные первые открытия БД сериализованы в изоляте,
push-изоляту генерация ключа БД запрещена, allow-list контактов читается с ретраями.
`flutter analyze` — 0 ошибок, `flutter test` — 392 passed (прогон по обоим коммитам сразу).

---

## 2026-07-17 — Корень ночного «слёта аккаунта»: повторная миграция secure storage после wipe

**Контекст:** утром 17.07 на устройстве (b35) — снова welcome-экран поверх живого аккаунта и пустая
история чатов. Файловый лог (15.07 18:41 – 17.07 06:21) вскрыл цепочку.

**Диагноз (проверен адверсариально, две итерации):**
1. 15.07 22:51:33 `performWipe` -> `prefs.clear()` стёр флаг `secure_storage_reset_v10_pkcs1`
   (одноразовая миграция v10, `ensureSecureStorageMigrated`, вызывается только в `main()`).
2. Аккаунт восстановили по бэкапу В ТОЙ ЖЕ сессии — флаг никто не восстановил; приложение жило
   без холодного старта до ночи 17.07.
3. Холодный старт 05:52: миграция увидела отсутствующий флаг -> молчаливый `deleteAll()` снёс
   seed и ключ БД («Ключи: НЕТ» за 16мс, ни одной ошибки — функция ничего не логировала).
4. 06:20 повторный импорт бэкапа (тот же адрес u6Z3, PoP ok) -> UI открыл БД -> ключа нет ->
   b35 удалил старый файл БД с историей 16.07 и создал пустой. История невосстановима.

**Грабли расследования (зафиксировать):** первая адверсариальная проверка «опровергла» диагноз,
опираясь на отсутствие в логе принтов `Identity imported/loaded` — а их и не могло быть: print из
tap-хендлеров выполняется в root-зоне, ВНЕ Zone-перехватчика (`runZonedGuarded` оборачивает только
`runApp`; gesture-пайплайн регистрируется раньше, в `ensureInitialized`). Прямая улика двойной
генерации ключа БД в одну мс на b35 доказала и то, что fcntl-лок не работает внутри процесса.

**Сделано (commit 1 — идентичность/миграция):**
- `secure_storage_options.dart`: `readSecureWithRetry` ретраит и `null` (ложный null Keystore),
  логирует чтение со 2+ попытки; `ensureSecureStorageMigrated` логирует каждую ветку;
  новый `markSecureStorageMigrated()` — флаг без deleteAll.
- `auth_service.dart` `performWipe`: восстановление флага миграции сразу после `prefs.clear()`;
  контроль полноты wipe (если seed пережил deleteAll — точечный повтор + честный репорт в errors).
- `crypto_service.dart`: `markSecureStorageMigrated()` после записи seed в `generateNewKeys`/
  `importPrivateKey` (закрывает класс целиком, включая будущие prefs.clear); print -> DebugLogger
  (зоно-независимая видимость create/import/load/delete в файловом логе); чтение даты регистрации
  только при живом seed; `hasStoredIdentity()` для контроля wipe.

**Сделано (commit 2 — защита БД):**
- `database_service.dart`: single-flight открытия БД (`_dbOpenFuture`, провал не кэшируется;
  сброс в `close()`/`initWithDatabase`) — закрывает внутрипроцессную гонку двойной генерации
  ключа; `allowDbKeyGeneration` (статик, пер-изолятно) — push-изоляту генерация запрещена;
  при отсутствии ключа файл БД ОТКЛАДЫВАЕТСЯ в `<db>.orphaned` (один слот), а не удаляется,
  с громким логом «существовал: true/false»; wipe уносит и orphan; allow-list контактов
  читается с ретраями (ложный null = молчаливый дроп звонка в фоне); комментарий про
  «отдельный процесс» исправлен (изолят в том же процессе, fcntl внутри процесса не лочит).
- `notification_service.dart` (`handleBackgroundPush`) и `push_connection_service.dart`
  (`pushConnectionOnStart`): `DatabaseService.allowDbKeyGeneration = false` на входе изолята.

**Проверки:** `flutter analyze` — 0 ошибок (warnings старые), `flutter test` — 392 passed.
Device-чеклист (wipe -> восстановление в той же сессии -> force-stop -> холодный старт: ключи
ЕСТЬ, история на месте, в логе «флаг на месте, сброс не требуется») — на владельце, сборка b36.
