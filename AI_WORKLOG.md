...
**Остаётся как РЕШЕНИЕ ВЛАДЕЛЬЦА, не дефект:** звонок под duress теряется без следа — после входа
настоящим PIN нет ни пропущенного в переписке, ни уведомления, а звонивший видит «не ответили».
Это ровно тот класс потери, который для сообщений закрыли парковкой (`parkPersonalChatUntilRealSession`).
Для звонка парковать сам offer бессмысленно (TTL 60с, SDP протухает), но след сделать можно:
отложить маркер и после входа настоящим PIN записать «Пропущенный звонок» — это обычная запись
переписки с текстом `Missed call` (chat_message_model.dart:45), сеть не задействована. Вопрос
задан владельцу.

**Статус:** analyze — 0 ошибок; test — 438 passed (было 436).
