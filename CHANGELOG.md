...
- Локальные уведомления (входящий звонок / новое сообщение / имя по умолчанию) и подписи панели звонка (Отклонить/Ответить/Микрофон/Динамик/Завершить) — **двуязычные (EN/RU)** через gen-l10n, без жертвы английским. Панель звонка берёт строки из `L10n.of(context)`; уведомления идут из фонового FCM-изолята без `BuildContext`, поэтому язык резолвится по сохранённому выбору (`app_locale`) → системная локаль → английский. Добавлены ключи l10n: `answerCall`, `microphone`, `newMessage`, `unknownCaller`.
