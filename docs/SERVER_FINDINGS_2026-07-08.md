...
## Готовый патч мелких фиксов — `docs/SERVER_SMALL_FIXES.patch`

Применяется из корня серверного репо: `git apply SERVER_SMALL_FIXES.patch` (проверено
`py_compile`; изменения минимальны и обратно совместимы; ни один тест не ссылается на
затронутые поля). Что внутри:
- **Приватность:** `internal_notes` больше не отдаётся в публичном `/api/public/releases`
  и `/releases/latest` (`app/public_api.py`).
- **Паритет оплаты:** вотчер шлёт license-status через `notify_license_activated`
  (fan-out + Redis + offline), а не «руками» (`main.py`, `payments.py`).
- **Фикс `tx_id`-коллизии:** уникальный `tx_id` на инвойс (`f"trx_auto_{payment_id}"`) —
  раньше константа блокировала активацию всех, кроме первого (`payments.py`).
- Косметика: стухший комментарий `sleep(60)` (`payments.py`).

**Отдельно (не в патче — `requirements.txt` в кодировке UTF-16, чтобы не портить диффом):**
добавить строку `bip_utils==2.9.3` (сверить версию) в блок «Blockchain». Без неё
`payments.py` падает на импорте при `PAYMENTS_ENABLED=1`. Заодно можно перекодировать
`requirements.txt` в UTF-8 (некоторые версии pip плохо едят UTF-16).

**Что патч НЕ делает** (осознанно — крупная работа, отдельной сборкой): 3 REST-эндпоинта
оплаты, USDT TRC-20-вотчер, solidified/анти-фрод, тарифы. См. раздел №1 выше.
