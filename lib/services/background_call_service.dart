...
class BackgroundCallService {
  ...
  ///
  /// `start()` перед этим — не лишний вызов, а страховка: микрофонный сервис теперь
  /// делит уведомление с сервисом доставки и при остановке ОТЦЕПЛЯЕТ его
  /// (STOP_FOREGROUND_DETACH), а не удаляет. Если сервис доставки в этот момент мёртв,
  /// отцеплённое уведомление зависло бы в шторке навсегда — некому перевыложить.
  /// `start()` идемпотентен.
